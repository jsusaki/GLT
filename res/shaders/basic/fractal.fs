...
{
    vec2 z;
    z.x = (texcoords.x - 0.5) * (1.0 / zoom) * aspect * 2.0 + center_offset.x;
    z.y = (texcoords.y - 0.5) * (1.0 / zoom)          * 2.0 + center_offset.y;         
...
{
    vec2 c, z;
    c.x = (texcoords.x - 0.5) * (1.0 / zoom) * aspect * 2.0 + center_offset.x - 0.5;
    c.y = (texcoords.y - 0.5) * (1.0 / zoom)          * 2.0 + center_offset.y;         
    z = vec2(0.0);
...
void main()
{
    ...
    // mouse control
    //vec2 mouse_normalized = mouse / screen_size;
    //vec2 mouse_ndc = mouse_normalized * 2.0 - 1.0;
    vec2 center_offset;
    //center_offset.x = (mouse_normalized.x - 0.5) * zoom * aspect * 2.0 + center_offset.x;
    //center_offset.y = (mouse_normalized.y - 0.5) * zoom * 2.0          + center_offset.y;
